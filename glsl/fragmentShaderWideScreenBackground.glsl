...
vec4 getSampleFromArray(sampler2D arrayTexture[NUMSYMB], int ndx, vec2 uv) {
    ...
    vec3 edgeSample(vec4 tex, vec2 uv)
    {
...
vec4 colorSecond = getSampleFromArray(arrayTexture, int(textureIndex) >= 7 ? 0 : int(textureIndex) + 1, vec2(vUv.x, vUv.y + t));
    ...
    if (boolRotate) {
        //float f = sin(t*2.0);
        float f = sin(timeRotate * speedFactor*2.0);
        float uvK = 3.0;
        if (f >= -1.0 && f <= -0.75) {
                colorMain = texture2D(arrayTexture[7], vec2(vUv.x, vUv.y + t)*uvK);
                colorSecond = texture2D(arrayTexture[0], vec2(vUv.x, vUv.y + t)*uvK);
        } else if (f >= -0.75 && f < -0.5) {
            colorMain = texture2D(arrayTexture[6], vec2(vUv.x, vUv.y + t)*uvK);
            colorSecond = texture2D(arrayTexture[1], vec2(vUv.x, vUv.y + t)*uvK);
        } else if (f >= -0.5 && f < -0.25) {
            colorMain = texture2D(arrayTexture[5], vec2(vUv.x, vUv.y + t)*uvK);
            colorSecond = texture2D(arrayTexture[2], vec2(vUv.x, vUv.y + t)*uvK);
        } else if (f >= -0.25 && f < 0.0) {
            colorMain = texture2D(arrayTexture[4], vec2(vUv.x, vUv.y + t)*uvK);
            colorSecond = texture2D(arrayTexture[3], vec2(vUv.x, vUv.y + t)*uvK);
        } else if (f >= 0.0 && f < 0.25) {
            colorMain = texture2D(arrayTexture[3], vec2(vUv.x, vUv.y + t)*uvK);
            colorSecond = texture2D(arrayTexture[4], vec2(vUv.x, vUv.y + t)*uvK);
        } else if (f >= 0.25 && f < 0.5) {
            colorMain = texture2D(arrayTexture[2], vec2(vUv.x, vUv.y + t)*uvK);
            colorSecond = texture2D(arrayTexture[5], vec2(vUv.x, vUv.y + t)*uvK);
        } else if (f >= 0.5 && f < 0.75) {
            colorMain = texture2D(arrayTexture[1], vec2(vUv.x, vUv.y + t)*uvK);
            colorSecond = texture2D(arrayTexture[6], vec2(vUv.x, vUv.y + t)*uvK);
        } else if (f >= 0.75 && f <= 1.0) {
            colorMain = texture2D(arrayTexture[0], vec2(vUv.x, vUv.y + t)*uvK);
            colorSecond = texture2D(arrayTexture[7], vec2(vUv.x, vUv.y + t)*uvK);
        } else {
               colorMain = texture2D(arrayTexture[7], vec2(vUv.x, vUv.y + t)*uvK);
               colorSecond = texture2D(arrayTexture[0], vec2(vUv.x, vUv.y + t)*uvK);
        }
...
/*#ifdef USE_GLITCH
    ...
    if (boolHolo) {
       ...
       c += Scanline(edgeSample((1.0-gl_FragColor)*0.35, uv)*1.5, uv);
